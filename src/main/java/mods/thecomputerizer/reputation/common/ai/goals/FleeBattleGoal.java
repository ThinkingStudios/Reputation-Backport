...
public class FleeBattleGoal extends Goal {
    ...
    public boolean canUse() {
        if(this.mob.getLastHurtByMob() instanceof Player player) {
            if(ensureCorrectConditions(player)) {
                ...
                Vec3 vec3 = RandomPos.getPosAvoid(this.mob, 32, 16, this.fleeFrom.position());
                if (vec3 == null) {
                    return false;
    ...
    @Override
    public void stop() {
        if(!this.mob.isDeadOrDying() && !ensureCorrectConditions(this.fleeFrom)) PacketHandler.sendTo(new FleeIconMessage(mob.getUUID(), false), (ServerPlayer) this.fleeFrom);
        this.fleeFrom = null;
    }
